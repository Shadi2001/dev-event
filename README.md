...
- **[SQLite](https://www.sqlite.org/)** is a lightweight, file-based database that runs entirely locally. No external database server is required—everything lives in a single file (`prisma/dev.db`), making setup and development simple.

- **[Prisma](https://www.prisma.io/)** is an ORM for Node.js and TypeScript. It provides type-safe database access, migrations, and a local SQLite setup so you can develop without any external database.
...
Copy the example env file and add your local values:
...
Ensure `.env` and `.env.local` include at least:
...
Optional: [**Cloudinary**](https://jsm.dev/devevent-cloudinary) (for POST `/api/events` image uploads), [**PostHog**](https://jsm.dev/devevent-posthog).

**Local database (SQLite + Prisma)**

No external database required. Create and seed the local SQLite DB:
